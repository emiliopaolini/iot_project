...
static void res_get_handler(coap_message_t *request, coap_message_t *response, uint8_t *buffer, uint16_t preferred_size, int32_t *offset){
    ...
    if (accept == APPLICATION_JSON) {
        ...
        snprintf((char *)buffer, COAP_MAX_CHUNK_SIZE, "{\"value\":%.1f}", light_level);
...
static void res_event_handler(){
    ...
    if(actuatorStatus==0){
	printf("actuator is off.. light is decreasing\n");
    	light_level += randInRange(-2,0);
    }
    else{
	printf("actuator is on.. light is increasing\n");
	light_level += randInRange(2,4.5);
    }
    
    //make sure that the changes will be between MINIMUM_LIGHT_LEVEL and MAXIMUM_LIGHT_LEVEL
    

    if(light_level>=MAXIMUM_LIGHT_LEVEL)
	    light_level = MAXIMUM_LIGHT_LEVEL;

    if(light_level<=MINIMUM_LIGHT_LEVEL)
	    light_level = MINIMUM_LIGHT_LEVEL;
    //notify coap clients subscribed to this resource
    coap_notify_observers(&light_measuring_device);
}
